...
const assetMetadata = {
  ...
  "GIF 3.gif": { title: "Interactive Motion Graphics Sequence", subtitle: "Motion Graphics // After Effects", impact: "Elevating Brand Motion" },
...
function App() {
  ...
  const [activeTab, setActiveTab] = useState('all') // 'all', 'gifs', 'pngs'
  ...
  const cursorXSpring = useSpring(cursorX, { damping: 30, stiffness: 180, mass: 0.6 })
  const cursorYSpring = useSpring(cursorY, { damping: 30, stiffness: 180, mass: 0.6 })
  ...
  const filteredWork = allWorkItems.filter(item => {
    ...
    if (activeTab === 'pngs') return item.type === 'png'
    return true
  })
  ...
  return (
    <>
        ...
        <AnimatePresence>
          {cursorMode === 'close' && (
            <motion.span
              ...
              className="text-[10px] font-mono tracking-widest text-black font-black uppercase pointer-events-none select-none"
            >
              Close
            </motion.span>
          )}
      ...
      <AnimatePresence>
        {selectedWork && (
          <div className="fixed inset-0 z-[990] flex items-center justify-center pointer-events-none w-screen h-screen">
            <motion.div
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
              ...
              className="absolute inset-0 bg-black/70 backdrop-blur-xl pointer-events-auto cursor-none"
            />

            {/* Morphing expanded wrapper */}
            <motion.div
              layoutId={`work-media-${selectedWork.id}`}
              className="relative z-10 max-w-[90vw] max-h-[80vh] w-auto h-auto pointer-events-auto rounded bg-neutral-950 border border-neutral-900 overflow-hidden shadow-[0_24px_80px_rgba(0,0,0,0.9)]"
              transition={{ duration: 0.2, ease: "easeOut" }}
            >
            ...
            {/* Typographic Caption overlay at bottom center */}
            <motion.div
              initial={{ opacity: 0, y: 15 }}
              animate={{ opacity: 1, y: 0 }}
              exit={{ opacity: 0, y: 15 }}
              transition={{ duration: 0.15, ease: "easeOut" }}
              className="absolute bottom-10 left-1/2 transform -translate-x-1/2 text-center text-white z-20 pointer-events-none w-full px-6"
            >
              <h4 className="font-serif text-xl sm:text-2xl font-bold tracking-tight mb-2">
                {selectedWork.title}
              </h4>
              {selectedWork.impact && selectedWork.subtitle ? (
                <p className="font-mono text-[10px] text-neutral-400 tracking-wider">
                  {selectedWork.impact} &bull; {selectedWork.subtitle}
                </p>
              ) : (
                <p className="font-mono text-[10px] text-neutral-400 tracking-wider">
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6 py-6 md:py-10">
        ...
        <header className="flex justify-end items-center mb-10 md:mb-16">
          <button 
            ...
            className="font-mono text-[10px] tracking-wider text-neutral-400 hover:text-white transition-colors cursor-none focus:outline-none flex items-center gap-2"
          >
            <span>{theme === 'dark' ? 'Light mode' : 'Dark mode'}</span>
            <span className="w-2 h-2 rounded-full bg-white border border-neutral-600 inline-block" />
          </button>
        </header>

        {/* Hero Section (Split Screen Architecture) */}
        <section className="mb-24 md:mb-36 h-auto md:h-[70vh] min-h-[500px] w-full relative bg-black overflow-hidden">
          <div className="grid grid-cols-1 md:grid-cols-2 h-full gap-12 md:gap-8">
            ...
            <div className="flex flex-col justify-center h-full text-left order-2 md:order-1 pr-4 md:pr-16 lg:pr-20 z-10">
              ...
              <div className="flex flex-wrap items-center gap-4 sm:gap-6 mt-8">
                <a 
                  ...
                  onMouseEnter={() => setCursorHovered(true)}
                  onMouseLeave={() => setCursorHovered(false)}
                  className="px-6 py-3 font-sans font-bold text-sm tracking-wider transition-all duration-300 cursor-none inline-block hero-btn-primary"
                >
                  Explore work
                </a>
                <a 
                  ...
                  onMouseEnter={() => setCursorHovered(true)}
                  onMouseLeave={() => setCursorHovered(false)}
                  className="px-6 py-3 border font-sans text-sm tracking-wider transition-all duration-300 cursor-none hero-btn-secondary"
                >
                  Download CV
                </a>
              </div>
            ...
            <div className="relative h-[60vh] md:h-full w-full order-1 md:order-2 bg-black overflow-hidden flex items-center justify-center will-change-transform">
              {/* Isolated Portrait in grayscale */}
              <img 
                src={heroPortrait} 
                alt="Manoj Manoharan Portrait" 
                decoding="async"
                style={{
                  maskImage: 'linear-gradient(to bottom, black 75%, transparent 100%)',
                  WebkitMaskImage: 'linear-gradient(to bottom, black 75%, transparent 100%)'
                }}
                className={`object-cover w-full h-full relative z-10 pointer-events-none select-none transition-all duration-500 ${
                  theme === 'dark' ? 'grayscale' : 'grayscale-0'
                }`}
              />
            </div>
        ...
        {/* Section divider */}
        <div className="w-full h-[1px] bg-neutral-900 my-24" />

        {/* Selected Work Section (Visual Masonry Grid) */}
        <section id="works" className="mb-40 md:mb-56">
          <div className="flex flex-col md:flex-row md:items-end justify-between mb-20 gap-8">
            ...
            <div className="flex gap-4 font-mono text-xs">
              {[
                { id: 'all', label: 'All' },
                { id: 'gifs', label: 'Motion (GIFs/MP4s)' },
                { id: 'pngs', label: 'Design (WebPs)' }
              ].map(tab => (
                <button
                  key={tab.id}
                  onClick={() => setActiveTab(tab.id)}
                  className={`hover-link-underline pb-1 tracking-wider transition-colors cursor-none ${
                    activeTab === tab.id 
                      ? 'text-white' 
                    ...
                    >
                      ...
                      <div className="mt-4 flex flex-col flex-grow">
                        <div className="flex justify-between items-baseline gap-4 mb-2">
                          ...
                          <span className="font-mono text-[9px] text-neutral-500 tracking-wider shrink-0">
                            {item.impact}
                          </span>
                        </div>
                        <div className="mt-auto flex justify-between items-center text-[10px] font-mono text-neutral-400 pt-2 border-t border-neutral-900/50">
                          <span>{item.subtitle}</span>
                          <span className="text-neutral-600">0{currentPage * itemsPerPage + index + 1}</span>
                        </div>
                      </div>
                    </motion.div>
                  )
          ...
          {totalPages > 1 && (
            <div className="flex justify-between items-center mt-16 pt-8 border-t border-neutral-900 font-mono text-xs select-none">
              <button
                ...
                onMouseEnter={() => setCursorHovered(true)}
                onMouseLeave={() => setCursorHovered(false)}
                className={`hover-link-underline pb-1 tracking-wider transition-all duration-300 cursor-none ${
                  currentPage === 0 
        ...
        {/* Experience Section */}
        <section id="roadmap" className="mb-40 md:mb-56">
          <div className="mb-24">
            <span className="font-sans text-neutral-500 text-[10px] tracking-wider block mb-3">
            ...
            {roadmap.map((item, index) => {
              ...
              return (
                <div 
                  ...
                  className="border-b border-neutral-900 py-10 md:py-14 cursor-none select-none"
                >
                  <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                    ...
                    <span className="font-mono text-xs text-neutral-500 tracking-wider">{item.duration}</span>
                  </div>

                  {/* Sharp Details Reveal on Hover */}
                  <AnimatePresence>
                    {isHovered && (
                      <motion.div
                      ...
                      >
                        <div className="grid grid-cols-1 md:grid-cols-12 gap-8 pt-8 mt-8 border-t border-neutral-900/50">
                          <div className="md:col-span-4">
                            <span className="font-mono text-[10px] text-neutral-600 tracking-wider block mb-1">Role Title</span>
                            <span className="text-white text-lg font-serif font-bold italic">{item.role}</span>
                          </div>
                          
                          <div className="md:col-span-8">
                            <span className="font-mono text-[10px] text-neutral-600 tracking-wider block mb-1">Focus & Responsibility</span>
                            <p className="text-neutral-400 text-sm font-light leading-relaxed max-w-2xl mb-6">
                              {item.description}
                            </p>
                            
                            <div className="space-y-2">
                              <span className="font-mono text-[10px] text-neutral-600 uppercase tracking-widest block mb-1">Key Outcomes</span>
        ...
        <Suspense fallback={<div className="h-48 w-full flex items-center justify-center font-mono text-xs text-neutral-500">Loading visual studies...</div>}>
          <LensSection 
            jpegs={jpegs} 
            skewY={skewY} 
        ...
        <Suspense fallback={<div className="h-48 w-full flex items-center justify-center font-mono text-xs text-neutral-500">Loading social directory...</div>}>
          <SocialDirectory 
        ...
        {/* Footer info */}
        <footer className="text-center font-mono text-[10px] text-neutral-600 pt-8 border-t border-neutral-900 flex flex-col md:flex-row justify-between items-center gap-4 tracking-wider">
